...
from utils.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, check_requirements, \
    box_iou, non_max_suppression, scale_coords, xyxy2xywh, xywh2xyxy, set_logging, increment_path, colorstr, oks_iou_fast
...
def test(data,
    ...
    jdict, stats_det, ap, ap_class, wandb_images = [], [], [], [], []
    stats_kpt = [] if kpt_label else None
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            t = time_synchronized()
            # print('before',np.array(out[0].cpu()).shape)
            # print(out[0][0].cpu())
            out = non_max_suppression(out, conf_thres, iou_thres, labels=lb, classes=None, multi_label=True, agnostic=single_cls, kpt_label=kpt_label, nc=model.yaml['nc'], nkpt=model.yaml['nkpt'], multi_cls_offset=False)
            # print('after',np.array(out[0].cpu()).shape)
        ...
        for si, pred in enumerate(out):
            ...
            if len(pred) == 0:
                if nl:
                    stats_det.append((torch.zeros(0, niou, dtype=torch.bool), torch.Tensor(), torch.Tensor(), tcls))
                    if kpt_label:
            ...
            correct_det = torch.zeros(pred.shape[0], niou, dtype=torch.bool, device=device)
            correct_kpt = torch.zeros(pred.shape[0], niou, dtype=torch.bool, device=device) if kpt_label else None
            if nl:
                detected = []  # target indices
                detected_kpt = []  # target indices
                    ...
                    if pi.shape[0]:
                        ...
                        for j in (ious > iouv[0]).nonzero(as_tuple=False):
                            ...
                            if d.item() not in detected_set:
                                ...
                                detected.append(d)
                                correct_det[pi[j]] = ious[j] > iouv  # iou_thres is 1xn
            ...
            # Append statistics (correct_det, conf, pcls, tcls)
            stats_det.append((correct_det.cpu(), pred[:, 4].cpu(), pred[:, 5].cpu(), tcls))
            if kpt_label:
                stats_kpt.append((correct_kpt.cpu(), pred[:, 4].cpu(), pred[:, 5].cpu(), tcls))
            # print(correct_det[0].cpu())
            # print(pred[0].cpu())
            # print('____')
        ...
        if plots and batch_i < 3000:
            ...
            # print(output_to_target(out)[0], output_to_target(out)[0].shape)
            # print('_________________')
            # print(save_dir / f'{path.stem}_labels.jpg')
    ...
    # Compute statistics
    stats_det = [np.concatenate(x, 0) for x in zip(*stats_det)]  # to numpy
    if len(stats_det) and stats_det[0].any():
        p, r, ap, f1, ap_class = ap_per_class(*stats_det, plot=plots, save_dir=save_dir, names=names)
        ap50, ap = ap[:, 0], ap.mean(1)  # AP@0.5, AP@0.5:0.95
        mp, mr, map50, map = p.mean(), r.mean(), ap50.mean(), ap.mean()
        nt = np.bincount(stats_det[3].astype(np.int64), minlength=nc)  # number of targets per class
    else:
        nt = torch.zeros(1)
    ...
    mp_1, mr_1, map50_1, map_1 = mp, mr, map50, map

    # Print results per class
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats_det):
    ...
    if kpt_label:
        ...
        mp_2, mr_2, map50_2, map_2 = mp, mr, map50, map

        # Print results per class
        if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats_kpt):
            for i, c in enumerate(ap_class):
                print(pf % (names[c], seen, nt[c], p[i], r[i], ap50[i], ap[i]))
    ...
    if save_json or save_json_kpt and len(jdict):
        ...
        if save_json:
            ...
            try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
                ...
                eval.accumulate()
                eval.summarize()
                map, map50 = eval.stats_det[:2]  # update results (mAP@0.5:0.95, mAP@0.5)
    ...
    if kpt_label:
        return (mp_1, mr_1, map50_1, map_1, mp_2, mr_2, map50_2, map_2, *(loss.cpu() / len(dataloader)).tolist())
    else:
        return (mp_1, mr_1, map50_1, map_1, *(loss.cpu() / len(dataloader)).tolist())
